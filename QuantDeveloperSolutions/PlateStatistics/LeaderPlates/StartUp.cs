...
public class StartUp
{
	...
	static void Main(string[] args)
	{
		...
		while(curDate<endDate){
			...
			Console.WriteLine("条件1：{0}，条件2：{1}",eleArray[0],eleArray[1]);
			QueryDocument query = new QueryDocument(eleArray);
		
			List<BsonDocument> listBD=new List<BsonDocument>();	
			MongoCollection<BsonDocument> collection = database.GetCollection<BsonDocument>("IndustriesUpLimitStatis");
			foreach (BsonDocument document in collection.Find(query)){
				listBD.Add(document);
			}
			collection = database.GetCollection<BsonDocument>("AreasUpLimitStatis");
			foreach (BsonDocument document in collection.Find(query)){
				listBD.Add(document);
			}
			collection = database.GetCollection<BsonDocument>("ConceptsUpLimitStatis");
			foreach (BsonDocument document in collection.Find(query)){
				listBD.Add(document);
			}
			...
			foreach(BsonDocument document in listBD){
				...
				if (!document.TryGetValue("area",out bsonValue)){
					if (!document.TryGetValue("concept",out bsonValue)){
						if (!document.TryGetValue("industry",out bsonValue)){
							continue;
						}
					}
				}
