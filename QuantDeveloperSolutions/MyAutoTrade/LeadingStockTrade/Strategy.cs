...
public class Strategy{
	...
	public void Start(){
		...
		/*//下面为自己添加，主要是为了模拟执行者在购买下单时能进行资金检查
		if (this.executionProvider is SimulationExecutionProvider)
		{
			SimulationExecutionProvider simulationExecutionProvider=this.executionProvider as SimulationExecutionProvider;
			simulationExecutionProvider.AccountValue=this.portfolio.GetAccountValue();//同步账户资金
			simulationExecutionProvider.CheckBuyPower=this.checkBuyPower;//检查购买力
			simulationExecutionProvider.ClearOrderCashFlows();//清除订单在模拟执行中的预扣现金流
		}
		//自己添加，为了让模拟数据提供者读取按月分区存储的数据序列
		if (this.marketDataProvider is SimulationDataProvider){
			SimulationDataProvider simulationDataProvider=this.marketDataProvider as SimulationDataProvider;
			simulationDataProvider.MonthPartitionStore=true;
			simulationDataProvider.PartitionStoreDirectory="e:/QDData";
			simulationDataProvider.SeriesNamesToPartitionStore="Trade,Quote";
		}*/
	...
	protected List<Marketor> marketors=new List<Marketor>();
	protected void AddMarketor(Marketor marketor){
		if (!this.marketors.Contains(marketor)) this.marketors.Add(marketor);
	}
	protected Dictionary<Instrument,List<Behavior>> behaviorDict=new Dictionary<Instrument,List<Behavior>>();
	protected void AddBehavior(Instrument instrument,Behavior behavior){
